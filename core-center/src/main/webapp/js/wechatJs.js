...
function config(appId, nonceStr, timestamp, signature, apiList, coins) {
    setupConfig(appId, nonceStr, timestamp, signature, apiList);
    wx.ready(function () {
...
function setupConfig(appId, nonceStr, timestamp, signature, apiList) {
    wx.config({
...
function scanDeviceInfo(appId, nonceStr, timestamp, signature, callback) {
    setupConfig(appId, nonceStr, timestamp, signature, ['scanQRCode']);
    wx.ready(function () {
        // config信息验证后会执行ready方法，所有接口调用都必须在config接口获得结果之后，config是一个客户端的异步操作，所以如果需要在页面加载时就调用相关接口，则须把相关接口放在ready函数中调用来确保正确执行。对于用户触发时才调用的接口，则可以直接调用，不需要放在ready函数中。
        wx.scanQRCode({
            needResult: 1, // 默认为0，扫描结果由微信处理，1则直接返回扫描结果，
...
function locateTheDevice(fullPath) {
    var param = fullPath.substring(fullPath.indexOf('#') + 1);
    if (param.indexOf("=") > -1) {
